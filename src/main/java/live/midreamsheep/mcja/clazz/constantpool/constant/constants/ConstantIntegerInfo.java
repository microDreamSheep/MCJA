...
public class ConstantIntegerInfo implements MCJAConstant {
    private byte[] data;

    private static final ConstantEnum type = ConstantEnum.CONSTANT_INTEGER;

    @Override
    public boolean setData(byte[] data) throws IllDataTypeException{
        if(data.length!=4){
            throw new IllDataTypeException("CONSTANT_INTEGER need four byte data");
        }
        this.data = data;
        return true;
    }
